...
async def create_index(config: IndexConfig):
    ...
    try:
        if es_client.index_exists(config.index_name):
            raise HTTPException(
                status_code=400, detail=f"Index '{config.index_name}' already exists"
            )
...
async def index_document(
    index_name: str, document: DocumentModel, doc_id: Optional[str] = None
):
    ...
    try:
        ...
        for chunk in document_chunks:
            ...
            response = es_client.index_document(
                index_name, doc_dict, doc_id, embeddings=embeddings
            )
...
async def update_document(
    index_name: str, doc_id: str, update_data: Dict[str, Any] = Body(...)
):
...
@app.post("/api/documents/parseDocument")
...
async def advanced_search(
    ...
    size: int = Query(settings.DEFAULT_SEARCH_SIZE, ge=1, le=settings.MAX_SEARCH_SIZE),
